...
OUT = r'C:\Users\bromine\Desktop\Git Projects\bromine1997.github.io\static\images\algorithm\da-push-flowchart.png'
...
# ── 레이아웃 좌표 ──────────────────────────────────────────────────
# YES 브랜치(4요소): center-to-center 간격 1.2 적용
# NO 브랜치(단순): 긴 수직선으로 자연스럽게 연결
...
Y_START = 10.00
Y_COND1 =  8.70
Y_CAP2  =  8.70   # 조건1과 같은 y — 수평 YES 화살표
Y_RALL  =  7.30   # Y_CAP2 - 1.40 (gap 충분)
Y_COND2 =  5.90   # Y_RALL - 1.40
Y_ERR   =  5.90   # 조건2와 같은 y — 수평 YES 화살표
Y_DATA  =  4.50   # Y_COND2 - 1.40
Y_PUSH  =  2.50
Y_END   =  1.10

# ── 도형 그리기 ────────────────────────────────────────────────────

b_start = make_oval(MX, Y_START, w=3.0, h=0.70)
txt(MX, Y_START, 'da_push() 시작', fs=10.5)

d1 = make_diam(MX, Y_COND1, w=2.8, h=1.00)
txt(MX, Y_COND1, 'size == capacity?')

b_cap2 = make_rect(RX, Y_CAP2, w=3.0, h=0.68)
txt(RX, Y_CAP2, 'capacity × 2')

b_rall = make_rect(RX, Y_RALL, w=3.8, h=0.68)
txt(RX, Y_RALL, 'temp = realloc(data, capacity)', fs=9.0)

d2 = make_diam(RX, Y_COND2, w=2.6, h=0.96)
txt(RX, Y_COND2, 'temp == NULL?')

b_err = make_rect(ERX, Y_ERR, w=2.2, h=0.76)
txt(ERX, Y_ERR + 0.14, '에러 반환', fs=9.2)
txt(ERX, Y_ERR - 0.18, '(data 유지)', fs=8.8)

b_data = make_rect(RX, Y_DATA, w=3.0, h=0.68)
txt(RX, Y_DATA, 'data = temp')

b_push = make_rect(MX, Y_PUSH, w=3.4, h=0.68)
txt(MX, Y_PUSH, 'data[size++] = value')

b_end = make_oval(MX, Y_END, w=2.6, h=0.66)
...
label((d1['right'] + b_cap2['left']) / 2, d1['cy'] + 0.24, 'YES')
...
label((d2['right'] + b_err['left']) / 2, d2['cy'] + 0.24, 'YES')

# 조건2 NO → data=temp
arr(d2['cx'], d2['bot'], b_data['cx'], b_data['top'])
label(d2['cx'] + 0.40, (d2['bot'] + b_data['top']) / 2, 'NO')

# data=temp → 합류점 JY (L자: 수직 ↓ → 수평 ← → 수직 ↓)
JY = b_push['top'] + 0.55
...
label(d1['cx'] - 0.42, d1['bot'] - 0.35, 'NO')
