...
기존 고압산소치료(HBOT) 시스템은 도입 비용이 높고 전문 설치 환경이 필요한 데다, **실시간 원격 관리 기능의 부재** 탓에 병원 밖에서는 활용하기 어려웠다. 의료진이 늘 상주해야 했고 원격 제어·모니터링도 불가능했다. 자동화된 프로세스가 부족해 운영 효율성도 낮았다.
...
일반적인 Android 앱은 스마트폰에서 실행되는 UI다. 이 프로젝트에서는 챔버에 부착된 Tinker Board 2S(소형 컴퓨터) 위에서 Android 앱이 직접 실행되며, 앱 자체가 **챔버를 제어하는 컨트롤러** 역할을 한다. MRAA 라이브러리로 앱 코드에서 하드웨어 핀(GPIO·SPI·I2C·UART)을 직접 읽고 쓸 수 있어, 센서 수집 → PID 제어 → 서버 전송까지 단일 앱 안에서 모두 처리한다.
...
- Hardware Control: MRAA (GPIO, SPI, I2C, UART)
...
| 항목                    | 측정값          | 비고                         |
| ----------------------- | --------------- | ---------------------------- |
| 최대 운용 압력          | 약 3기압        | 3.02 ATA 실험값 기준         |
...
| 35분 프로파일 overshoot | 미세한 범위 내 안정 | 응급 배기 상황 포함      |
| 3시간 장기 프로파일     | 목표 압력 대비 근소한 초과 | 응급 배기 상황 포함   |
...
Step Function 실험에서 확인한 미미한 Overshoot와 응급 배기밸브 테스트 결과는 **PID 제어 시스템의 안정성**을 보여준다.
...
이 프로젝트는 Tinker Board 2S가 Android OS를 지원한다는 점을 활용해 Android 앱 자체를 컨트롤러로 운용했다. MRAA 라이브러리로 Java 코드에서 GPIO 핀을 직접 제어하므로, 제어 로직·UI·통신을 Android Studio 단일 환경에서 통합 개발할 수 있다. 이것이 이 선택의 핵심 이유다.
...
본 연구는 IoT 기술로 고압산소치료 시스템의 한계를 완화하고, **병원 중심에서 개인 맞춤형 헬스케어로 전환할 가능성**을 제시했다.
