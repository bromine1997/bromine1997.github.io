...
- 앱 크래시 시 밸브 fail-safe 처리 없음 — 가압 중 크래시 발생 시 밸브가 열린 채로 멈출 수 있음
- WebSocket 연결 끊김 감지 없음 — 원격 모니터링 중 연결이 끊겨도 대시보드에서 인지 불가
- 센서 이상값 필터링 없음 — 센서 오작동 시 PID가 이상값 기준으로 제어
- 응급 배기 감지 시 PID 인터럽트 미구현 — 배기 상태 감지 후 PID 정지 로직 추가 가능
