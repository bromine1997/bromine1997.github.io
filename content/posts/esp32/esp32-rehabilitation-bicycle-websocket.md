...
캘리브레이션까지 끝나면 센서 데이터가 실제 무게(kg)로 나온다. 이제 이 데이터를 어떻게 밖으로 꺼내느냐가 남는다. 유선 시리얼은 측정 중에 케이블이 걸리적거리고, Bluetooth는 지연이 있다. Wi-Fi + WebSocket을 선택했다.
...
IoT 장비에서 웹 UI를 제공하는 방식은 크게 세 가지로 나뉜다.
...
40 SPS로 최대 36분을 측정하면 PSRAM에 쌓이는 구조체 배열이 약 86,400개다. 이걸 한 번에 WebSocket으로 보내면 TCP 버퍼를 초과한다.
...
이 경우 웹 서버도 WebSocket도 뜨지 않는다. 시리얼 모니터로만 데이터를 확인할 수 있다. 캘리브레이션 작업이나 센서 단순 확인 시 사용한다.
