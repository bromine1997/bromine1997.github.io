...
![ADS1232 오실레이터 연결 구성](/images/esp32/ads1232-oscillator-diagram.png)
...
처음 납땜할 때는 ADS1232 보드를 일렬로 배치했다. 작동은 했지만 배선이 지저분하고 그라운드 배선이 불안정했다.

![1차 하드웨어 보드](/images/esp32/rehab-pcb-board-v1.jpeg)
*1차 시도 — ADS1232 4개를 세로로 배치. 배선이 비효율적이고 노이즈에 취약한 구조였다.*

수정하면서 ESP32를 중앙에 두고 ADS1232 4개를 상하좌우로 대칭 배치했다. 각 채널의 배선 길이가 균일해지면서 노이즈도 줄었다.

![2차 하드웨어 보드](/images/esp32/rehab-pcb-board.jpeg)
*2차 수정본 — 대칭 배치로 배선 정리, 그라운드 강화. 최종 탑재 버전.*
