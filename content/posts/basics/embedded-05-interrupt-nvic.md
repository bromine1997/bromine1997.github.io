...
CPU가 주기적으로 상태를 직접 확인한다. 단순하지만 문제가 있다.

- CPU가 루프를 돌며 GPIO를 계속 읽어야 한다
- 짧은 펄스나 빠른 이벤트를 놓칠 수 있다
- 확인할 이벤트가 늘어날수록 반응 속도가 느려진다
...
- **폴링**: CPU가 직접 확인. 단순하지만 CPU 낭비, 이벤트 누락 가능
- **인터럽트**: 하드웨어가 CPU에 알림. 빠르고 효율적
- **NVIC**: Cortex-M 인터럽트 컨트롤러. 우선순위 0~15, 숫자 낮을수록 높음
- **Preemption Priority**: 현재 핸들러를 중단하고 끼어들 수 있는지 결정
- **ISR 원칙**: 짧게. 플래그만 세우고 실제 처리는 메인 루프에서
- **volatile**: 인터럽트와 공유하는 변수에는 반드시. 최적화 빌드에서 누락 시 버그 발생

다음 편에서는 보드레이트와 통신 속도의 근원인 클럭 트리와 RCC를 정리한다.
