...
가압·유지 구간에서는 가압 밸브 전류만 조절한다. 챔버 구조상 공기가 조금씩 자연 누설되기 때문에, 목표압력에 도달한 후에도 가압 밸브를 열어 누설분을 보충해야 압력이 유지된다. 감압 구간에서는 반대로 배기 밸브를 열어 압력을 낮춘다. "압력을 보고 → 해당 구간에 맞는 밸브 전류를 조절 → 다시 압력이 바뀜"이라는 폐루프가 만들어진다.
