...
> **역사 메모**: PID 제어의 첫 이론 논문은 Nicolas Minorsky가 1922년 발표한 *"Directional Stability of Automatically Steered Bodies"*다. 미 해군 함정의 자동 조타 시스템을 수학적으로 분석하면서 비례·적분·미분 항의 개념이 정립되었다.
...
가압·유지 구간에서는 가압 밸브 전류만 조절한다. 챔버 구조상 가만히 있어도 공기가 조금씩 빠져나가기 때문에, 목표압력에 도달한 후에도 가압 밸브를 열어 빠져나간 만큼 채워줘야 압력이 유지된다. 감압 구간에서는 반대로 배기 밸브를 열어 압력을 낮춘다. "압력을 보고 → 해당 구간에 맞는 밸브 전류를 조절 → 다시 압력이 바뀜"이라는 폐루프가 만들어진다.
...
압력을 올리는 동역학(가압 밸브 → 산소 유입)과 내리는 동역학(배기)은 특성이 다르다. 하나의 PID로 양방향을 다루기보다 가압+유지용과 감압용 PID를 분리하는 편이 깔끔하다. 각 구간에 맞는 게인을 따로 둘 수 있기 때문이다. 상황에 따라 제어기를 바꿔 쓰는 것을 **모드 스위칭(switching control)** 또는 넓게는 게인 스케줄링이라 부른다.
...
- 가압 밸브와 배기 밸브의 특성이 달라 가압/감압 동작이 비대칭이다.
...
- N. Minorsky, "Directional Stability of Automatically Steered Bodies," *Journal of the American Society of Naval Engineers*, vol. 34, no. 2, pp. 280–309, 1922.
- J. G. Ziegler and N. B. Nichols, "Optimum Settings for Automatic Controllers," *Transactions of the ASME*, vol. 64, no. 8, pp. 759–765, 1942.
...
- L. Gracia et al., "Identification and control of a multiplace hyperbaric chamber," *PLOS One*, 2018.
