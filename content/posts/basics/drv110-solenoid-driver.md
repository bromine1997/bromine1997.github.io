...
HBOT에서는 이 특성이 안전 설계와 직결된다. 치료 중 예기치 않게 전원이 차단되면 N.C. 밸브는 즉시 닫힌다. 가압 상태에서 감압 경로의 밸브가 닫히면 챔버 내 압력을 빼낼 수 없어 환자가 고압 상태에 갇히게 된다. 이 때문에 감압 경로의 솔밸브는 **전원이 끊겨도 열린 상태를 유지하거나 자동으로 열릴 수 있어야 한다**. 실제로는 UPS나 커패시터 백업으로 순간 정전에 대응하고, 수동 감압 밸브를 별도로 두어 비상 경로를 확보하는 방식을 함께 사용했다.
