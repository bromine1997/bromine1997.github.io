...
$$E = 14\text{W} \times \frac{100}{60}\text{h} \approx 23.3\text{Wh (세션당)}$$
...
부품값이 잘못되면 밸브가 제대로 동작하지 않는다. 실패 유형별로 정리하면 다음과 같다.

| 부품 | 잘못된 설정 | 증상 |
|------|-----------|------|
| RPEAK | 너무 높음 (IPEAK 부족) | 피크 전류가 pull-in force를 못 이겨 밸브가 열리지 않음 |
| RPEAK | 너무 낮음 (IPEAK 과다) | 코일 과열, 소손 위험 |
| RHOLD | 너무 높음 (IHOLD 부족) | 홀드 전류가 낮아 플런저가 도중에 떨어짐 (밸브 의도치 않게 닫힘) |
| RHOLD | 너무 낮음 (IHOLD 과다) | 홀드 구간에도 높은 전류 → 피크-앤-홀드 효과 반감 |
| CKEEP | 너무 작음 (tKEEP 부족) | 밸브가 완전히 열리기 전에 홀드 전류로 전환 → 개방 실패 |
| CKEEP | 너무 큼 (tKEEP 과다) | 피크 전류 구간이 필요 이상으로 길어져 발열 증가 |
