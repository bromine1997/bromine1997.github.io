...
HBOT에서는 이 특성이 안전 설계와 직결된다. 치료 중 예기치 않게 전원이 차단되면 N.C. 밸브는 즉시 닫힌다. 가압 상태에서 감압 경로의 밸브가 닫히면 챔버 내 압력을 빼낼 수 없어 환자가 고압 상태에 갇히게 된다. 이 때문에 솔밸브와 별도로 비상 감압 밸브를 두어 전원 이상 시에도 챔버 압력을 빼낼 수 있는 경로를 확보했다.
