...
VXZ240FGE는 **상시 열림(Normally Open, N.O.)** 타입이다. 전원이 없으면 스프링 힘으로 열려 있고, 코일에 전류를 공급해야 닫힌다.

HBOT에서는 이 특성이 안전 설계와 직결된다. 치료 중에는 코일에 전류를 흘려 밸브를 닫아 압력을 유지한다. 만약 치료 중 예기치 않게 전원이 차단되면 N.O. 밸브는 즉시 열린다. 즉, 전원 차단만으로도 감압 경로가 자동으로 확보되어 챔버 압력이 빠져나갈 수 있다. **N.O. 밸브는 전원 이상 시 자연스럽게 안전 상태(열림)로 돌아가는 페일세이프(fail-safe) 설계**다.

여기서 핵심 포인트가 있다.

- **당기는 순간(Pull-in)**: 플런저가 스프링 힘을 이기고 이동해야 하므로 큰 자기력이 필요하다 → **높은 전류 필요**
- **붙어 있는 동안(Hold)**: 이미 이동한 플런저를 닫힌 위치에 붙들어 두기만 하면 된다. 자기 회로 에어갭이 줄어 효율이 올라가므로 → **훨씬 낮은 전류로 충분**
...
1. **피크 구간**: 밸브를 처음 닫을 때만 정격 전류(292mA)를 짧게 공급한다
2. **홀드 구간**: 플런저가 닫힌 위치에 고정되고 나면 낮은 전류(예: 100mA)로 떨어뜨려 유지한다
