...
학부 때 ESP32로 재활 자전거 IoT 모니터링 시스템을 만들고, 석사에서는 Tinker Board 2S와 STM32를 써서 HBOT 챔버 제어 시스템을 구축했다. 그 과정에서 SPI, PID 제어, 디지털 아이솔레이터, NestJS 백엔드까지 꽤 다양한 것들을 다뤘다.

근데 기술 스택을 쌓는 데 집중하다 보면 정작 가장 기본적인 개념들이 흐릿해지기 마련이다. "임베디드가 뭔지", "펌웨어가 뭔지" — 이런 질문일수록 더 탄탄하게 설명할 수 있어야 한다고 생각했다.
...
- 자동차 ECU
- 산업용 로봇 제어기
- 의료기기 (내가 석사 연구에서 만든 HBOT 챔버 제어 시스템도 여기에 해당한다)
...
이유는 간단하다. STM32는 **ARM Cortex-M** 코어, ESP32는 **Xtensa LX6** 코어다. CPU 아키텍처가 다르면 명령어 체계(ISA, Instruction Set Architecture)가 달라지기 때문에, 같은 C 코드라도 컴파일된 기계어가 전혀 다르다. 여기에 레지스터 구조, 엔디언(Endianness), 메모리 맵까지 다르기 때문에 펌웨어는 특정 하드웨어에 완전히 종속될 수밖에 없다.
...
- **RTOS 기반**: FreeRTOS 같은 실시간 운영체제 위에서 동작. 태스크 우선순위, 자원 관리가 필요한 경우 사용한다.
...
내 HBOT 챔버 제어 시스템의 경우, 압력과 산소 농도를 일정 주기로 안정적으로 제어해야 했기 때문에 실시간성이 요구됐다.
...
MCU, DSP와 근본적으로 다르다. 엄밀히 말하면 FPGA는 "회로를 만든다"기보다 **논리 소자(Logic Gates)들의 연결(Routing)을 재설정**하는 방식으로 동작한다. 이 특성을 **하드웨어 재구성 가능성(Reconfigurability)** 이라고 부른다.

MCU는 내가 짠 코드를 CPU가 순서대로 실행하는 방식이다. FPGA는 HDL(Hardware Description Language)로 회로를 기술하면 내부 논리 소자들의 연결이 그에 맞게 재구성된다.

덕분에 **병렬처리**가 가능하다. 수천 개의 픽셀을 동시에 처리해야 하는 영상처리, 초고속 통신 프로토콜 처리 등에서 MCU보다 압도적으로 유리하다.

단점은 명확하다. 개발 난이도가 높고 비용이 비싸다. STM32에서 UART 하나 구현하는 건 레지스터 몇 개 설정하면 끝이지만, FPGA에서는 UART 회로 자체를 만들어야 한다.

| 구분 | MCU | DSP | FPGA |
|---|---|---|---|
| 처리 방식 | 순차처리 | 순차처리 | 병렬처리 |
| 특화 분야 | 범용 제어 | 신호처리 | 고속 병렬 처리 |
| 개발 난이도 | 낮음 | 중간 | 높음 |
| 비용 | 저렴 | 중간 | 비쌈 |
