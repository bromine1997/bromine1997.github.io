...
6편에서 클럭 트리 구조를 잡았다. SYSCLK(168 MHz)가 APB1(42 MHz)과 APB2(84 MHz)로 분주되고, UART/SPI/I2C가 각 APB에 연결된다는 것까지 정리했다.

이번 편에서는 실제 보드레이트가 어떻게 계산되는지 파고든다. `115200`이라는 숫자의 근거, 오차가 얼마인지, SPI 속도는 어떻게 고르는지.
...
- **UART BRR**: `f_PCLK / (16 × USARTDIV)`. 소수점 반올림으로 오차 발생, ±2% 이내면 정상 통신 가능
- **같은 설정이라도 UART 번호에 따라 APB가 다르다**: UART2 → APB1(42MHz), UART1 → APB2(84MHz)
- **SPI 분주비**: 2의 거듭제곱만 지원. 장치 최대 클럭보다 낮게 내림 선택
- **보드레이트 오류 진단**: 오실로스코프로 실제 비트 주기 측정
- **SPI 간헐 오류**: 분주비를 낮춰서(속도 절반) 먼저 테스트

다음 편부터는 실제 통신 프로토콜로 들어간다. 클럭 트리를 이해한 상태에서 UART/SPI/I2C를 보면 보드레이트 설정의 근거가 명확해진다.
