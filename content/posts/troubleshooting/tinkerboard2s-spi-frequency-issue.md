...
다만 **SPI5(MAX1032)** 쪽은 500kHz로는 부족했고, **20kHz**까지 낮춰야 정상 파형이 나왔다. 같은 보드에서 두 SPI 라인이 이렇게 큰 차이를 보이는 정확한 원인까지는 규명하지 못했다. 다만 오실로스코프로 파형을 직접 확인하며 신호 무결성이 보장되는 주파수까지 보수적으로 낮춰 적용했다.
