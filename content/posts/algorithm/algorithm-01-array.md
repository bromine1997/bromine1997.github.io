...
C++의 `std::vector`, Java의 `ArrayList`가 바로 이 구조의 구현체다. 내부적으로 동일하게 힙 할당 + Doubling Strategy를 사용한다. 우리가 직접 만들어보는 것이다.
...
- Doubling Strategy로 `realloc()` 호출 횟수를 O(log n)으로 줄일 수 있다. 이것이 `std::vector`, `ArrayList`의 내부 동작 원리이기도 하다.
